...
function initParticles() {
  ...
  requestAnimationFrame(function () {
    try {
      // Cache viewport width once to avoid layout thrashing
      const viewportWidth = window.innerWidth;

      // Use shared device detection from utils.js (loaded via index.html)
      const utils = window.PortfolioUtils || {};
      const isMobile = utils.isMobile ? utils.isMobile() : viewportWidth < 768;
      const isTablet = utils.isTablet ? utils.isTablet() : (viewportWidth >= 768 && viewportWidth < 1024);
